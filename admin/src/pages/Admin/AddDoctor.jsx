...
const AddDoctor = () => {
  ...
  const onSubmitHandler = async (event) => {
    event.preventDefault() //to prevent page reload on submitting
    ...
    try {
      ...
      // api call to add data in db
      const {data} = await axios.post(backendUrl + '/api/admin/add-doctor', formData, {headers:{aToken}})
      //in backend, authAdmin, I used atoken, so it will converted in lowercase, no issue
      if(data.success){
      ...
      <div className='bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
         <div className='flex items-center gap-4 mb-8 text-gray-500'>
          <label htmlFor='doc-img'>
           ...
           {/*console.log(URL.createObjectURL(docImg)) -> create temporary localhost url for the img*/} 
          
          </label>
          <input onChange={(e)=> setDocImg(e.target.files[0])} type='file' id='doc-img' hidden />
          {/* onChange={(e)=> setDocImg(e.target.files[0])} -> image will be stored in docImg when selct any image  */}
